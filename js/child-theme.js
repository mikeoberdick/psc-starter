	...
	*/
	...
	jQuery(function ($) {
	  $(document).ready(function () {
	    /////////////////  PUSH DOWN FOOTER  \\\\\\\\\\\\\\\\\
	    $('#js-heightControl').css('height', $(window).height() - $('html').height() + 'px'); ///////////////// CLOSE THE MOBILE NAV LINK ON LINK TO A PAGE  \\\\\\\\\\\\\\\\\
	    ...
	    if ($(window).width() >= 992) {
	      ...
	      $('.offset-right').css('padding-right', Math.ceil(offset.left) + 15 + 'px'); //Force a section to be the width of column plus the gutter (left or right)

	      var pushToLeftWidth = $('.push-to-left-edge').outerWidth();
	      $('.push-to-left-edge .outer-wrapper').css('width', Math.ceil(offset.left) - 15 + pushToLeftWidth + 'px'); //Don't subtract the 15 pixels for padding due to the checkerboard pattern on the image above
	      ...
	      $('.push-to-right-edge').each(function () {
	        ...
	        $(this).find('.outer-wrapper').css({
	          'width': Math.ceil(offset.left) - 15 + pushToRightWidth + 'px',
	          'height': height
	        });
	      });
	    ...
	    $('.hero .inner-container').css('margin-top', headerHeight / 2 + 'px'); ///////////////// DROPDOWN HOVER ON DESKTOP  \\\\\\\\\\\\\\\\\
	    ...
	    $('#homepage #sectionFour .bucket .icon-wrapper').matchHeight(); //RUN AFTER ALL ELEMENTS INCLUDING IMAGES
